...
export class ExamService {
  ...
  submitExam(submissionData: any): Observable<ExamResult> {
    return this.http.post<ExamResult>('http://localhost:9090/api/exam-results/submit', submissionData);
  }
  ...
  getMyExams(userId: number): Observable<Exam[]> {
    ...
    return this.http.get<Exam[]>('http://localhost:9090/api/exam-enrollments/my-exams', { params });
  }

  completeCourse(courseId: number, userId: number): Observable<string> {
    const params = new HttpParams().set('userId', userId.toString());
    return this.http.post<string>(`http://localhost:9090/api/courses/${courseId}/complete`, null, { params });
  }
}
